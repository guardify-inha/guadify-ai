...
CONTRACTS_USER_DIR = os.path.join(DATA_ROOT, "contracts", "user")
...
def main():
    ...
    if os.path.isdir(CONTRACTS_USER_DIR):
        for f in os.listdir(CONTRACTS_USER_DIR):
            if f.lower().endswith(".pdf"):
                file_list.append((os.path.join(CONTRACTS_USER_DIR, f), "user"))
