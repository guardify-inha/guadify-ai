"""
실제 약관 조항 형태의 종합 테스트 실행 및 결과 파일 생성
"""
import sys
...
def run_realistic_test():
    """실제 약관 조항 형태의 종합 테스트 실행"""
    print("=" * 80)
    print("📊 실제 약관 조항 형태 종합 테스트 실행")
    print("=" * 80)
    
    # 테스트 케이스 로드
    test_cases_path = Path(__file__).parent / "realistic_test_cases.json"
    ...
    # 상세 결과를 파일로 저장 (각 케이스별 상세 정보 포함)
    detailed_results = []
    for i, (test_case, result) in enumerate(zip(test_cases, results), 1):
        detailed_results.append({
            "번호": i,
            "테스트_문장": test_case["query"],
            "예상_결과": test_case["expected"],
            "실제_결과": result.get("actual", "N/A"),
            "예측_성공_여부": "성공" if result.get("correct", False) else "실패",
            "통합_점수": round(result.get("integrated_score", 0), 3),
            "LLM_위반_가능성": round(result.get("llm_probability", 0), 3)
        })
    
    # CSV 형식으로 저장
    output_csv = Path(__file__).parent.parent / "docs" / "comprehensive_test_results.csv"
    ...
    print(f"\n📄 상세 결과 CSV 저장: {output_csv}")
    
    # JSON 형식으로도 저장
    output_json = Path(__file__).parent.parent / "docs" / "comprehensive_test_results.json"
    ...
    print(f"📄 상세 결과 JSON 저장: {output_json}")
...
def visualize_results(article_stats, overall_accuracy, total_correct, total_count):
    ...
    for pattern, count in wrong_patterns.most_common(10):
